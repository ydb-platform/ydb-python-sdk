# -*- coding: utf-8 -*-
import time

import jwt

from ydb.oauth2_token_exchange import JwtTokenSource

PRIVATE_KEY = "-----BEGIN PRIVATE KEY-----\\nhL8bFAuNNVrCOp79TNnNIsh7\n-----END PRIVATE KEY-----\n"  # noqa: E501
PUBLIC_KEY = "-----BEGIN PUBLIC KEY-----\\njQIDAQAB\n-----END PUBLIC KEY-----\n"  # noqa: E501


def check_exception(f):
    try:
        f()
    except Exception:
        return
    assert False, "Exception is expected"


def test_no_signing_method():
    def f():
        JwtTokenSource(signing_method="", private_key=PRIVATE_KEY, key_id="key_id")

    check_exception(f)


def test_no_private_key():
    def f():
        JwtTokenSource(signing_method="RS256", private_key="", key_id="key_id")

    check_exception(f)


def test_incorrect_ttl():
    def f():
        JwtTokenSource(signing_method="RS256", private_key=PRIVATE_KEY, key_id="key_id", token_ttl_seconds=0)

    check_exception(f)


def test_jwt_encode():
    aud = ["aud1", "aud2"]
    src = JwtTokenSource(
        signing_method="RS256",
        private_key=PRIVATE_KEY,
        key_id="key_id",
        subject="subject",
        audience=aud,
    )
    jwt_token = src.token()
    assert jwt_token.token_type == "urn:ietf:params:oauth:token-type:jwt"

    decoded = jwt.decode(jwt_token.token, key=PUBLIC_KEY, algorithms=["RS256"], audience=aud)
    assert decoded.get("iss") is None
    assert decoded["sub"] == "subject"
    assert decoded["aud"] == aud
    assert abs(decoded["iat"] - time.time()) <= 60
    assert abs(decoded["exp"] - time.time()) <= 3600
