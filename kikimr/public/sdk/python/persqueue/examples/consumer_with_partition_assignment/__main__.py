...
# To enable manual partition assignment, specify this option in configurator
configurator = pqlib.ConsumerConfigurator(args.topic, args.consumer, use_client_locks=True)
...
# There may also be no new messages in topic if no one writes anything single last run of this sample.
...
while total_messages_expected > 0 or last_received_cookie != last_committed_cookie:
    ...
    try:
        ...
        raise RuntimeError(
            "Failed to get any messages form topic {}. Are your sure there is any data to read?".format(args.topic)
        )
    ...
    if result.type == pqlib.ConsumerMessageType.MSG_DATA:
        ...
        # This means that server has assigned some partition to our consumer.
        # Data from partition will be read only after confirmation from client done be calling ready_to_read() method

        # Here we just instantly confirm any assignment but you can do some preparation or processing if required and
        # confirm assignment later
        result.ready_to_read()
        print "Got partition assignment: topic {}, partition {}".format(
            result.message.lock.topic, result.message.lock.partition
        )
    elif result.type == pqlib.ConsumerMessageType.MSG_RELEASE:
        # That means that server revoked an assignment and were are not getting any data from this partition anymore
        # (Unless it's assigned again latter).
        # NOTICE: once receiving this signal it's recommended to commit() all data read from this partition
        # as soon as processed. In this sample we commit everything instantly, so this is not the case.
        print "Partition revoked. Topic {}, partition {}".format(
