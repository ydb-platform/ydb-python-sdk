...
def send_requests(client, lock, method, requests_queue, responses_queue, session_id, timeout):
    try:
        with lock:
            responses_iter = client.invoke(requests_generator(requests_queue), method=method, timeout=timeout)
    except Exception as e:
        logger.debug("Send connection failed marker - exception in send requests")
        responses_queue.put(
            ResponseReceivedEvent(
                session_id=session_id,
                response=SessionFailureResult(
                    reason=SessionDeathReason.HostConnectionFailed,
                    description="Init session request failed with exception: {}. Suppose connection troubles".format(e)
                )
            )
        )
        return
    ...
    logger.debug("Session %s created", method)
    try:
        ...
        if e.code() == grpc.StatusCode.UNAVAILABLE:
            logger.debug("Send connection failed marker - got unavailable code")
            reason = SessionDeathReason.HostConnectionFailed
...
class GRPCSingleSessionClient(object):
    def __init__(self, client, client_lock, method, responses_queue, my_session_id, timeout=None):
        self.__requests_queue = queue.Queue()
        self.__main_thread = threading.Thread(
            target=send_requests,
            args=(client, client_lock, method, self.__requests_queue, responses_queue, my_session_id, timeout),
            name="PQ lib grpc stream thread"
        )
        self.__main_thread.daemon = True
        self.session_id = my_session_id
